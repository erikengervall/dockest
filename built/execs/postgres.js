...
const createPostgres = (Logger) => {
    const startPostgresContainer = async (postgresConfig) => {
        ...
        await execa_1.default.shell(`docker-compose run --no-deps -d ${dockerComposeFile} --label ${postgresConfig.label} -p ${postgresConfig.port}:5432 ${postgresConfig.service}`);
